...
- **Frame extraction**: for scroll-driven playback, render the mockup motion
  to video and extract frames (see `motion-cinematic.md` § Frame Sequence Format Guide).
...
Continue with [`asset-production.md`](asset-production.md) for cutout background strategy, frontend recipes, image-set continuity, and anchor rotation.
