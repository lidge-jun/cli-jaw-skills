...
| `../jaw-dev-uiux-design/references/design-trends.md` | Implementing a dated trend or checking whether a technique is stable | 2026-07-14 axis prevalence, technique maturity, signature status, risks, and re-crawl/promotion rules |
| `../jaw-dev-uiux-design/references/compositional-patterns.md` | Calibrating anti-slop rules against award evidence | Context-gated hero, navigation, motion, typography, and media patterns with accessibility gates |
...
| `references/core/asset-production.md`     | Generated cutouts and image sets     | Background strategy, frontend recipes, continuity, and anchor rotation             |
...
| `references/core/motion-scroll.md`        | Scroll-linked effects                | Horizontal stages, sticky stacks, frame sequences, video scrub, text/SVG/Lottie patterns |
| `references/core/motion-cinematic.md`     | Cinematic section choreography       | Portal, perspective, crossfade, wipe, card transition, GSAP, and reduced-motion gates |
| `references/core/motion-media.md`         | Generated motion media and final audit | Organic/product heroes, format choices, video generation, performance, reduced motion, and honesty |
...
### React and Next.js Version Gates

- React 19.2 features such as `<Activity>` and `useEffectEvent` require version detection; use React Compiler only when the repo enables it and measurement justifies the change.
- In Next.js 16, Turbopack is the default and Cache Components replace the old PPR flag. Validate and authorize Server Actions, and never cache session data without a user-scoped key.
- Vite, SPA-only React, React Native Web, and client-only embeds do not support Next.js Server Component assumptions; preserve their client architecture.

### Modern CSS and Build Tools

- Prefer container queries, `:has()`, subgrid, View Transitions with fallback, `text-wrap`, viewport units, logical properties, and shallow nesting before JS layout observers.
- Detect Vite 7 versus Vite 8 before editing config. Vite 8 uses Rolldown/Oxc by default and requires Node 20.19+ or 22.12+; do not add Rollup/esbuild-era bridges blindly.
- Record browser Baseline status and fallback needs for modern features used by the target audience.
- Prefer dev servers that expose browser/runtime diagnostics to the agent: Vite 8 console forwarding and Next.js 16 DevTools integration are useful before static speculation.

### Design System Detection (MANDATORY)

Before inventing tokens or components, inspect `package.json`, existing token/theme files, and the brief for Material, Carbon, Fluent, GOV.UK, USWDS, shadcn/ui, or another governing system. Use and theme the installed official system instead of recreating it. For shadcn/ui, inspect local source plus `components.json`, aliases, tokens, and registry conventions; remove demo-only variants and never hallucinate components.
