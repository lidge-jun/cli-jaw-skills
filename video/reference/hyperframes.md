...
3. Produce a design read: mood, palette, typography, density, and motion intensity.
4. Author or replace the HyperFrames `index.html`; do not render the untouched blank scaffold.
5. Run `hyperframes lint`.
6. Run `hyperframes inspect` or capture representative snapshots.
7. Render to MP4.
8. Validate with `ffprobe`.
9. Inspect frames for blank output, text clipping, poor hierarchy, and motion quality.
10. Record commands, artifact paths, and visual QA in devlog.
