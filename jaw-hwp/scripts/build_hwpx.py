...
Usage:
    ...
    # With metadata
    python build_hwpx.py --template gonmun --section my.xml --title "Title" --creator "Author" --output result.hwpx
"""
...
# Import pack.py for linesegarray strip + minify
sys.path.insert(0, str(SCRIPT_DIR))
from office.pack import pack as pack_with_strip
...
def pack_hwpx(input_dir: Path, output_path: Path) -> None:
    """Create HWPX archive using pack.py (strips linesegarray + minifies XML).

    Delegates to office/pack.py which handles:
    - linesegarray removal (layout cache that causes text compression bugs)
    - XML minification (matches Hancom's native single-line format)
    - mimetype as first ZIP entry with ZIP_STORED
    """
    pack_with_strip(
        str(input_dir),
        str(output_path),
