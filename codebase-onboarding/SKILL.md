...
1. **Use Glob and Grep for reconnaissance** — read selectively only for ambiguous signals. Avoid reading every file.
...
- **Keep CLAUDE.md under 100 lines** — focus on essential conventions only
- **Highlight only dependencies that shape how code is written** — skip routine library listings
- **Focus directory descriptions on non-obvious purpose** — skip explaining self-evident names like `src/`
- **Add structural insight beyond what the README covers** — avoid duplicating existing documentation
