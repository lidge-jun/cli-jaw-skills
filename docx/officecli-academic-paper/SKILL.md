---
# officecli: help-driven schema; verify with `officecli --help` and `officecli help docx ... --json`
...
2. Switch to the parent DOCX lightweight fallback path if the requested document can tolerate lower fidelity.
3. Stop or cancel.

Before switching to lightweight fallback, ask again and state which academic-paper features may be lost, such as TOC fidelity, equation handling, footnotes/endnotes, validation, or CJK/rhwp-aware behavior. If the user chooses lightweight mode, save that preference to memory for future Office work.
