...
python scripts/ooxml/repair.py unpacked/
# Returns: {"repaired": int, "details": [...]}
# Then re-pack: python scripts/ooxml/pack.py unpacked/ output.docx --original document.docx
```
...
```bash
# Add a single comment anchored to specific text
python scripts/comment.py input.docx output.docx --author "Agent" --text "Comment text" --anchor "target phrase"

# Add multiple comments from JSON
python scripts/comment.py input.docx output.docx --json comments.json
# JSON format: [{"author": "Agent", "text": "Comment", "anchor": "text to find"}, ...]
```

The script inserts `commentRangeStart`, `commentRangeEnd`, and `commentReference`
markup automatically. It also creates `comments.xml` relationships/content-types
...
This script currently uses direct OOXML processing with a post-assertion pass.
It does not provision a LibreOffice macro module for you.
