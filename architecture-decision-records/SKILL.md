...
1. **Initialize (first time only)** — if `docs/adr/` does not exist, ask the user for confirmation before creating the directory, a `README.md` seeded with the index table header (see ADR Index Format below), and a blank `template.md` for manual use. Only write files after explicit consent.
...
- **Reserve ADRs for significant architectural choices** — variable naming or formatting choices belong elsewhere
- **Keep context under 10 lines** — trim to the essential forces and constraints
- **Include at least one considered alternative** — "we just picked it" is insufficient rationale
- **When recording past decisions, note the original date** — distinguish backfills from live captures
- **Mark superseded decisions with a link to their replacement** — keep the ADR log current
