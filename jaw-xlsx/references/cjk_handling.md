...
> **Note:** the `scripts.ooxml.cjk_utils` helpers below (`auto_fit_columns`, `get_display_width`, `check_contrast`) are **reference implementations, not a shipped module** — `skills_ref/jaw-xlsx/scripts/` ships only `recalc.py`, `xlsx_cli.py`, `run_tests.py`. Treat the `from scripts.ooxml.cjk_utils import ...` lines as illustrative: inline the logic or implement the helper yourself.
