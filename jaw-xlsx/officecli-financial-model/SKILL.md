...
2. Switch to the parent XLSX lightweight fallback path if the requested model can tolerate lower fidelity.
3. Stop or cancel.

Before switching to lightweight fallback, ask again and state which financial-model features may be lost, such as Office-native validation, formula/table/chart fidelity, recalculation behavior, or CJK/rhwp-aware behavior. If the user chooses lightweight mode, save that preference to memory for future Office work.
