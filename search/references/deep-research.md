...
Canonical owner of the deep-research **method** for cli-jaw. The tiers in `search/SKILL.md`
...
normal ladder in `search/SKILL.md` however important it is. Nothing auto-escalates into
this protocol.
...
Apply the reader contract from `dev/references/reader-documents.md` here — the scope
...
Families map onto the lanes `search/SKILL.md` already defines — `official`,
...
The tiers in `search/SKILL.md` do the work, unchanged — it owns which tier fits a
...
Model-gated parallel research (2-4 lanes) is already defined in `search/SKILL.md`; this
...
The reader-facing report follows `dev/references/reader-documents.md`: answer first,
