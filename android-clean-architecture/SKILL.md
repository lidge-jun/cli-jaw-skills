...
**Rule**: `domain` depends only on `core` (or nothing). It contains pure Kotlin — no framework imports.
...
- Exposing database entities or DTOs to the UI layer — map to domain models first
- Putting business logic in ViewModels — extract to UseCases
- Using `GlobalScope` or unstructured coroutines — prefer `viewModelScope` or structured concurrency
- Fat repository implementations — split into focused DataSources
- Circular module dependencies — if A depends on B, B should not depend on A
