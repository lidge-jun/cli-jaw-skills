...
2. Switch to the parent PPTX lightweight fallback path if the requested deck can tolerate lower fidelity.
3. Stop or cancel.

Before switching to lightweight fallback, ask again and state which pitch-deck features may be lost, such as Office-native validation, theme/master inheritance, CJK/rhwp-aware behavior, or high-fidelity mutation. If the user chooses lightweight mode, save that preference to memory for future Office work.
