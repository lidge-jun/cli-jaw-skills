...
Use this skill whenever the request depends on accurate, up-to-date behavior of a library, framework, or API. Applies across any agent environment with Context7 MCP configured.
...
You must obtain a Context7-compatible library ID (format `/org/project` or `/org/project/version`) before querying docs. Call query-docs only after getting a valid library ID from this step.
...
Limit: keep query-docs and resolve-library-id calls to 3 per question. If the answer is unclear after 3 calls, state the uncertainty and use the best information available.
...
- **No sensitive data**: Redact API keys, passwords, tokens, and other secrets from any query sent to Context7. Review the user's question for secrets before passing it to resolve-library-id or query-docs.
