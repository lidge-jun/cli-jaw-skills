...
```sql
-- Preferred: filter on indexed columns first
SELECT *
...
-- Avoid: filtering on non-indexed columns first
SELECT *
FROM markets_analytics
WHERE volume > 1000
...
```sql
-- Prefer ClickHouse-specific aggregation functions
SELECT
...
- Avoid FINAL (merge data before query instead)
- Limit JOINs (denormalize for analytics)
- Avoid small frequent inserts (batch instead)
