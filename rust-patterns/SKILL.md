...
// Avoid: Cloning unnecessarily to avoid borrow checker
...
### Use `Result` and `?` in production code
...
### Avoid unsafe for these purposes

```rust
// Avoid: Using unsafe to bypass borrow checker
// Avoid: Using unsafe for convenience
// Avoid: Using unsafe without a Safety comment
// Avoid: Transmuting between unrelated types
```
...
// Avoid: .clone() to satisfy borrow checker without understanding why
let data = expensive_data.clone();
process(&original, &data);

// Avoid: Using String when &str suffices
fn greet(name: String) { /* should be &str */ }

// Avoid: Box<dyn Error> in libraries (use thiserror instead)
fn parse(input: &str) -> Result<Data, Box<dyn std::error::Error>> { todo!() }

// Avoid: Ignoring must_use warnings
...
**Rust version notes (June 2026)**: Rust 1.96.0 (May 2026); **2024 Edition** since 1.85.0 — refined RPIT lifetime capture, `gen` keyword reserved, updated `unsafe` rules (`unsafe_op_in_unsafe_fn` warning), `IntoIterator` for `Box<[T]>`, improved `if let` temporary scoping, Cargo Rust-version aware resolver. Migrate with `cargo fix --edition`.
