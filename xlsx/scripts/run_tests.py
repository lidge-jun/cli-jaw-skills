#!/usr/bin/env python3
"""Regression test runner for xlsx_cli.py.
...
CLI = SCRIPT_DIR / "xlsx_cli.py"

# (command, args, expected_file, compare_mode)
TEST_CASES: list[tuple[str, list[str], str, str]] = [
    # Validate
    ("validate", [str(FIXTURES_DIR / "broken_refs.xlsx"), "--json"],
     "validate_broken_refs.json", "json"),

    # Repair dry-run
    ("repair", [str(FIXTURES_DIR / "broken_refs.xlsx"), "--json"],
     "repair_broken_refs.json", "json"),

    # Formula audit
    ("formula-audit", [str(FIXTURES_DIR / "formula_errors.xlsx"), "--json"],
     "formula_audit_errors.json", "json"),
...
def run_cli(command: str, args: list[str], mode: str = "text") -> str:
    """Run xlsx_cli.py and capture output."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Regression test runner for xlsx_cli.py")
