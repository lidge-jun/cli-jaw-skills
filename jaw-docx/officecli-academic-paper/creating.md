...
Complete guide for building a formally structured Word document with TOC, equations, footnotes, and scholarly formatting. Follow this step by step. For general building blocks (paragraphs, runs, tables, images, lists, charts), see [docx creating.md](../creating.md).
...
For table building blocks (header rows, cell styling, merging), see [docx creating.md](../creating.md#tables----creation--basic-styling). Academic table recipe:
...
**"Page X of Y" pattern:** Currently there is no single-command way to produce "Page X of Y" on one line. The `fieldType=page` and `fieldType=numpages` each create their own paragraph. For a simple page number, use the pattern above. For "Page X of Y", see [docx creating.md](../creating.md#headers--footers) for raw-set workarounds, or accept the limitation and use a simple page number field.
...
REF field text is static until the user updates fields in Word (Ctrl+A, F9). Internal hyperlinks (`#anchor`) are not supported -- use REF field + bookmark instead. For other field types (page numbers, dates), see [docx creating.md](../creating.md#fields).
