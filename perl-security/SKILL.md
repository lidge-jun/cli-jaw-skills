...
```perl
# 1. Two-arg open with user data (command injection — critical)
open my $fh, $user_input;
# 2. String-form system (shell injection — critical)
...
Perl's flexibility requires discipline. Use taint mode for web-facing code, validate all input with allowlists, use DBI placeholders for every query, and encode all output for its context.
